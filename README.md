...
* 반응형웹 편하게 수정하려면 꼭 필요한 고정 너비 또는 높이가 아닌 이상 상대값단위(fr ,%)를 이용해서 디자인하는 것이 좋다.
* `img`태그로 요소를 삽입하면 CSS를 설정하지 않아도 **자동으로 이미지의 원본 px를 유지**하기 때문에 반응형에서 px을 가진 이미지 때문에 레이아웃이 깨지는 문제가 종종 발생한다. 해결법 => px -> 100% 변경
